...
// 1. ADICIONAR após a classe Color (aproximadamente linha 100):
...
public:
	...
	// 3. SUBSTITUIR o método createSurfacePoint na classe ReSTIRRenderer:
	
	SurfacePoint createSurfacePoint(float x, float y) const {
	    Vec3 rayOrigin(x - WIDTH/2, y - HEIGHT/2, 100); // Posição da câmera
	    Vec3 rayDir(0, 0, -1); // Direção do raio (para baixo)
	    
	    // Verificar interseções com esferas primeiro
	    ...
	    // Se há interseção com esfera, retornar ponto da esfera
	    if (closestSphere >= 0) {
	    ...
	    // Caso contrário, retornar ponto do plano xadrez
	    Vec3 position(x - WIDTH/2, y - HEIGHT/2, 0);
	    Vec3 normal(0, 0, 1);
